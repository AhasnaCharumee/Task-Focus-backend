...
function nextWeeklyOccurrence(from: Date, daysOfWeek: number[], intervalWeeks: number) {
...
function computeNextRun(lastRun: Date, recurrence: any) {
...
export function startRecurrenceWorker(pollIntervalMs = 60 * 1000) {
  ...
  const work = async () => {
    try {
      ...
      for (const tmpl of dueTemplates) {
        try {
          const r = tmpl.recurrence as any;
          if (!r) continue;
          ...
          if (lastRun && lastRun.getTime() === nextRun.getTime()) {
            ...
            tmpl.recurrence = tmpl.recurrence || {} as any;
            (tmpl.recurrence as any).lastRun = nextRun;
            (tmpl.recurrence as any).nextRun = computed as any;
            if ((tmpl.recurrence as any).endDate && computed && new Date(computed) > new Date((tmpl.recurrence as any).endDate)) {
              (tmpl.recurrence as any).enabled = false;
            }
            await tmpl.save();
            continue;
          }
...
export default startRecurrenceWorker;
