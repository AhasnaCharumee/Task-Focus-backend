...
// 🔹 Generate Focus Plan
export const generateFocusPlan = async (req: Request, res: Response) => {
  try {
...
// 🔹 Generate Motivation
export const generateMotivation = async (req: Request, res: Response) => {
  try {
...
export const generateGoalBreakdown = async (req: Request, res: Response) => {
  try {
    const { goal } = req.body;
    if (!goal) return res.status(400).json({ message: "Missing goal" });
    ...
    const parts: string[] = goal.split(/,| and |;|\.|\n/).map((s: string) => s.trim()).filter(Boolean);
    const subtasks = parts.slice(0, 6).map((p: string, i: number) => ({
...
export const analyzeSentiment = async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(texts)) return res.status(400).json({ message: "Missing texts array" });
    ...
    const out = texts.map((t: string) => {
      ...
      let score = 0;
      positive.forEach((w) => { if (low.includes(w)) score += 1; });
      negative.forEach((w) => { if (low.includes(w)) score -= 1; });
...
export const contextSuggestions = async (req: Request, res: Response) => {
  try {
    const { tasks } = req.body;
    if (!Array.isArray(tasks)) return res.status(400).json({ message: "Missing tasks array" });
    ...
    const scoreTask = (t: any) => {
      let score = 0;
      if (t.status === 'done') score -= 1000;
      if (t.dueDate) {
        const d = new Date(t.dueDate);
        const diffH = (d.getTime() - now.getTime()) / 3600000;
        if (diffH <= 24) score += 50;
        else if (diffH <= 72) score += 20;
      }
      if (t.labels && t.labels.includes('urgent')) score += 30;
      if (t.priority === 'high') score += 40;
      // boost short tasks
      if (t.estimateMinutes && t.estimateMinutes <= 30) score += 10;
      return score;
    };

    const scored = tasks.map((t: any) => ({ task: t, score: scoreTask(t) }));
