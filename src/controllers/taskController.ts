...
export const createTask: RequestHandler = async (req, res) => {
  try {
    ...
    let recurrenceObj: any = undefined;
    if (recurrence) {
      recurrenceObj = {
...
export const getTasks: RequestHandler = async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ user: userId }).sort({ createdAt: -1 }).populate("categories");
    res.json(tasks);
  } catch (err) {
  ...
  try {
    ...
    if (updateData.labels && !Array.isArray(updateData.labels)) {
      ...
      if (typeof updateData.labels === "string") {
        updateData.labels = updateData.labels.split(",").map((s: string) => s.trim()).filter(Boolean);
      } else {
    ...
    if (updateData.recurrence) {
      ...
      const normalizedRec: any = {};
      if (r.enabled !== undefined) normalizedRec.enabled = !!r.enabled;
      if (r.frequency) normalizedRec.frequency = r.frequency;
      if (r.interval !== undefined) normalizedRec.interval = Number(r.interval) || 1;
      if (r.daysOfWeek) normalizedRec.daysOfWeek = Array.isArray(r.daysOfWeek) ? r.daysOfWeek.map(Number) : [];
      if (r.startDate) normalizedRec.startDate = new Date(r.startDate);
      if (r.endDate) normalizedRec.endDate = new Date(r.endDate);
      if (r.nextRun) normalizedRec.nextRun = new Date(r.nextRun);
      updateData.recurrence = normalizedRec;
    }

    const updated = await Task.findOneAndUpdate({ _id: req.params.id, user: authReq.user._id }, updateData, { new: true });
    if (!updated) return res.status(404).json({ message: "Task not found or not owned by user" });
...
export const deleteTask: RequestHandler = async (req, res) => {
  try {
    ...
    const deleted = await Task.findOneAndDelete({ _id: req.params.id, user: authReq.user._id });
    if (!deleted) return res.status(404).json({ message: "Task not found or not owned by user" });
...
export const previewReminders: RequestHandler = async (req, res) => {
  try {
    const authReq = req as AuthRequest;
    if (!authReq.user || !authReq.user._id) return res.status(401).json({ message: "Unauthorized" });

    const task = await Task.findOne({ _id: req.params.id, user: authReq.user._id }).lean();
    if (!task) return res.status(404).json({ message: "Task not found" });

    const n = Math.max(1, Number(req.query.n) || 5);
    const reminders: number[] = Array.isArray(task.reminders) ? task.reminders.map(Number) : [];

    const occurrences: Array<{ occurrence: string; reminders: string[] }> = [];
    ...
    const addMonths = (date: Date, months: number) => { const d = new Date(date); const m = d.getMonth(); d.setMonth(m + months); return d; };
    const nextWeeklyOccurrence = (from: Date, daysOfWeek: number[], intervalWeeks: number) => {
      ...
      for (const dow of sorted) {
        if (dow > fromDow) return addDays(from, dow - fromDow);
      }
    ...
    const computeNext = (lastRun: Date, r: any) => {
      const frequency = r?.frequency;
      const interval = Number(r?.interval || 1);
      const daysOfWeek = r?.daysOfWeek || [];
      if (!frequency) return undefined;
      if (frequency === "daily") return addDays(lastRun, interval);
      if (frequency === "weekly") return nextWeeklyOccurrence(lastRun, daysOfWeek, interval);
      if (frequency === "monthly") return addMonths(lastRun, interval);
      return undefined;
    };
    ...
    if (task.recurrence && task.recurrence.enabled) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const next = computeNext(occ, task.recurrence as any);
        if (!next) break;
        cursor = next;
      }
...
function computeOccurrencesInRange(recurrence: any, start: Date, end: Date, maxOccurrences = 100) {
  const occurrences: Date[] = [];
  if (!recurrence || !recurrence.enabled) return occurrences;

  const addDays = (date: Date, days: number) => { const d = new Date(date); d.setDate(d.getDate() + days); return d; };
  const addMonths = (date: Date, months: number) => { const d = new Date(date); const m = d.getMonth(); d.setMonth(m + months); return d; };
  const nextWeeklyOccurrence = (from: Date, daysOfWeek: number[], intervalWeeks: number) => {
    if (!daysOfWeek || daysOfWeek.length === 0) return addDays(from, 7 * intervalWeeks);
    const sorted = [...new Set(daysOfWeek.map(Number))].sort((a, b) => a - b);
    const fromDow = from.getDay();
    for (const dow of sorted) {
      if (dow > fromDow) return addDays(from, dow - fromDow);
  ...
  while (cursor && count < maxOccurrences) {
    if (recurrence.endDate && new Date(cursor) > new Date(recurrence.endDate)) break;
...
export const getCalendarEvents: RequestHandler = async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ user: authReq.user._id }).lean();
...
export const getTaskIcal: RequestHandler = async (req, res) => {
  try {
    const authReq = req as AuthRequest;
    if (!authReq.user || !authReq.user._id) return res.status(401).json({ message: "Unauthorized" });
    ...
    if (task.recurrence && task.recurrence.enabled) {
      const r = task.recurrence as any;
